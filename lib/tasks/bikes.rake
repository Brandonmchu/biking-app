...
kijiji.each_pair do |category,catUrl|
  ...
  for i in 0..5
    ...
    @post = Bike.new(:title => title, :price => price, :url => url, :posted => imageUrl, :location => locationKijiji, :category => category)
    @post.save
  end
end
  ...
  docCraigs.css(".row").take(2).each do |bike|
    ...
    @post = Bike.new(:title => titleCraigs, :price => priceCraigs, :url => urlCraigs, :posted => imageUrlCraigs, :location => locationCraigs)
    @post.save
  end
